...
async def auto_report_to_owner(context: ContextTypes.DEFAULT_TYPE):
...
async def run():
    ...
    try:
        ...
        # ── Auto-report to owner every 4 minutes ──────────
        app.job_queue.run_repeating(
            auto_report_to_owner,
            interval=120,   # 2 minutes in seconds
            first=10,       # first run 10 seconds after startup
            name="auto_report_owner",
        )
        logger.info("📋 Auto-report job scheduled: every 2 minutes → owner")
