...
async def recover(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Pattern: optional badge emoji, rank number, dot, space, name, space, (count)
    # e.g.  👑 1. @kranthick (8)   or   ▫️ 4. Display Name (0)
    pattern = _re.compile(
        r"(?:[\U00010000-\U0010ffff]|[\u2600-\u27BF]|\S+)?\s*"  # optional badge
        r"\d+\.\s+"                                               # rank + dot
        r"(@?\S[\s\S]*?)\s+"                                      # name (greedy, may have spaces)
        r"\((\d+)\)"                                              # (ref_count)
    )
    ...
    for line in raw.splitlines():
        line = line.strip()
        m = _re.search(r"\d+\.\s+(@?\S[\s\S]*?)\s+\((\d+)\)\s*$", line)
        if not m:
            continue
