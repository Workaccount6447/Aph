...
async def show_dashboard(update: Update, context: ContextTypes.DEFAULT_TYPE, user=None):
    ...
    eligible = ref_count >= REQUIRED_REFERRALS

    status = (
        "🎉 *You are eligible for the giveaway!*"
        if eligible
        else f"⏳ You need *{remaining}* more referral(s) to qualify."
    )

    bar_filled = min(ref_count, REQUIRED_REFERRALS)
    bar = "🟢" * bar_filled + "⚪️" * (REQUIRED_REFERRALS - bar_filled)

    text = (
        f"👋 Hello, *{user.full_name}*!\n\n"
        f"🎁 *Giveaway Referral Bot*\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"Progress: {bar}\n"
        f"📊 Referrals: *{ref_count} / {REQUIRED_REFERRALS}*\n"
        f"{status}\n\n"
        f"🔗 *Your referral link:*\n`{ref_link}`\n\n"
        f"Share this link — referrals only count after your friend *joins the channel*."
    )
        ...
        try:
            await context.bot.send_message(
                ...
                text=(
                    ...
                    + ("\n\n🎉 *You're now eligible for the giveaway!*" if ref_count >= REQUIRED_REFERRALS else "")
                ),
                parse_mode="Markdown",
            )
        except TelegramError:
...
async def all_users_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    total = len(all_users)
    eligible_count = sum(1 for *_, refs in all_users if refs >= REQUIRED_REFERRALS)
    ...
    for page_num, chunk in enumerate(chunks, 1):
        ...
        if page_num == 1:
            ...
            lines.append(f"Total: *{total}*  |  🏆 Eligible: *{eligible_count}*  |  ⏳ Pending: *{total - eligible_count}*")
            lines.append("━━━━━━━━━━━━━━━━━━━━━")

        for i, (uid, username, full_name, ref_count) in enumerate(chunk, (page_num - 1) * CHUNK + 1):
            badge = "🏆" if ref_count >= REQUIRED_REFERRALS else "▫️"
...
async def listallpart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for page_num, chunk in enumerate(chunks, 1):
        ...
        for i, (uid, username, full_name, ref_count) in enumerate(chunk, (page_num - 1) * CHUNK + 1):
            badge = "🏆" if ref_count >= REQUIRED_REFERRALS else "⏳"
...
async def winners(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    eligible = db.get_eligible_users(REQUIRED_REFERRALS)
    if not eligible:
        await update.message.reply_text("😔 No eligible participants yet.")
        return

    lines = [f"🏆 *Eligible Winners ({len(eligible)})*\n━━━━━━━━━━━━━━━━━━━━━"]
    for uid, username, full_name, ref_count in eligible:
        uname_display = f"@{username}" if username else "—"
        lines.append(f"• {full_name} | {uname_display} | {ref_count} refs | `{uid}`")
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"⏳ Pending: *{total - eligible_count}*\n"
        f"🎯 Required Referrals: *{REQUIRED_REFERRALS}*",
        parse_mode="Markdown",
    )
