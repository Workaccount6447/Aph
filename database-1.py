...
class Database:
    ...
    def add_user(self, user_id: int, username: str, full_name: str, referrer_id=None):
        """Insert or update a real user.
        If a recovered (synthetic negative-ID) row exists for the same username,
        absorb its ref_count and delete it so the leaderboard data carries over.
        """
        recovered_ref_count = 0
        if username:
            ghost = self.conn.execute(
                "SELECT user_id, ref_count FROM users WHERE username = ? AND user_id < 0",
                (username,),
            ).fetchone()
            if ghost:
                recovered_ref_count = ghost[1]
                self.conn.execute("DELETE FROM users WHERE user_id = ?", (ghost[0],))

        self.conn.execute("""
            INSERT INTO users (user_id, username, full_name, referrer_id, ref_count)
            VALUES (?, ?, ?, ?, ?)
            ON CONFLICT(user_id) DO UPDATE SET
                username  = excluded.username,
                full_name = excluded.full_name,
                ref_count = MAX(users.ref_count, excluded.ref_count)
        """, (user_id, username, full_name, referrer_id, recovered_ref_count))
        self.conn.commit()
    ...
    def recover_user(self, username: str, full_name: str, ref_count: int):
        ...
        - If a real user (positive ID) with this username already exists → update ref_count.
        - Otherwise insert with a stable synthetic negative ID to be merged on next /start.
        """
        if username:
            real = self.conn.execute(
                "SELECT user_id FROM users WHERE username = ? AND user_id > 0",
                (username,),
            ).fetchone()
            if real:
                self.conn.execute(
                    "UPDATE users SET ref_count = ? WHERE user_id = ?",
                    (ref_count, real[0]),
                )
                self.conn.commit()
                return
