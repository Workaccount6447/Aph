...
async def handle_health(request: web.Request) -> web.Response:
    return web.json_response({"status": "ok", "service": "giveaway-bot"})


async def handle_index(request: web.Request) -> web.Response:
    return web.Response(text="Giveaway Bot is running ✅")
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner(uid):
        text = (
            ...
            "📋 Auto-report: /all sent to you every *2 minutes*\n\n"
            "🔑 *Admin Commands:*\n"
...
# ═══════════════════════════════════════════════════════════
#  AUTO REPORT — sends /all report to owner every 4 minutes
# ═══════════════════════════════════════════════════════════

async def auto_report_to_owner_task(bot):
    """Automatically sends the /all participant report to the owner every 2 minutes."""
    all_users = db.get_all_users()
    ...
    if not all_users:
        try:
            ...
            logger.warning(f"Auto-report: failed to notify owner: {e}")
        return
    ...
    for page_num, chunk in enumerate(chunks, 1):
        ...
        try:
            ...
            logger.warning(f"Auto-report: failed to send page {page_num} to owner: {e}")
            break
...
# ═══════════════════════════════════════════════════════════
#  /recover  — OWNER ONLY
#  Parses a pasted /all leaderboard block and writes every
#  entry into the database (upsert by username, preserving
#  real user IDs when present).
#
#  Usage:
#    /recover
#    <paste the full leaderboard text as a reply or in the
#     same message body after the command>
#
#  Accepted name formats on each participant line:
#    👑 1. @username (8)
#    ▫️ 4. Display Name (0)
# ═══════════════════════════════════════════════════════════
...
async def recover(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Accept text from: the command message itself (after /recover),
    # or as a reply to a message containing the leaderboard.
    raw = ""
    if context.args:
        ...
        await update.message.reply_text(
            "ℹ️ *Usage:*\n"
            "Either send `/recover <leaderboard text>` or reply to a leaderboard message with `/recover`.",
            parse_mode="Markdown",
        )
        return

    # Matches:  <rank>. <name> (<count>)
    # Works whether the text has real newlines OR is collapsed into one line.
    # Badges (👑 🥈 🥉 ▫️ etc.) are simply ignored — they sit between entries.
    ENTRY_RE = _re.compile(r"\d+\.\s+(.*?)\s+\((\d+)\)(?=\s|$)", _re.DOTALL)

    recovered = []
    for name_raw, count_str in ENTRY_RE.findall(raw):
        ...
        db.recover_user(username=username, full_name=full_name, ref_count=ref_count)
        recovered.append((name_raw, ref_count))

    if not recovered:
        await update.message.reply_text(
            "⚠️ No participant lines found.\n"
            "Make sure you paste the full `/all` output.",
            parse_mode="Markdown",
        )
        return

    lines = [f"✅ *Recovery complete — {len(recovered)} participant(s) written to DB*\n"]
    for name, count in recovered:
        lines.append(f"• {name} → {count} referral(s)")
    await update.message.reply_text("\n".join(lines), parse_mode="Markdown")


# ═══════════════════════════════════════════════════════════
#  MAIN  — runs web server + telegram polling concurrently
# ═══════════════════════════════════════════════════════════
...
async def run():
    ...
    try:
        ...
        # ── Auto-report to owner every 2 minutes ──────────
        async def _auto_report_loop():
            await asyncio.sleep(10)  # wait 10s after startup
            while True:
                try:
                    await auto_report_to_owner_task(app.bot)
                except Exception as e:
                    logger.warning(f"Auto-report loop error: {e}")
                await asyncio.sleep(120)  # 2 minutes

        asyncio.create_task(_auto_report_loop())
        logger.info("📋 Auto-report task started: every 2 minutes → owner")
